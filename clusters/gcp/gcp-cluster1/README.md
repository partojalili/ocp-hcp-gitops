# OpenShift Cluster: gcp-cluster1

This directory contains the configuration for the OpenShift cluster **gcp-cluster1** on Google Cloud Platform.

## Cluster Details

- **Cluster Name**: `gcp-cluster1`
...
Download the OpenShift installer for version `4.20`:
...
```bash
# Create secret
oc create secret generic gcp-cluster1-kubeconfig \
...
1. **A Record**: `*.apps.gcp-cluster1.jdpff.gcp.redhatworkshops.io` → `<ingress-ip>`
2. **A Record**: `api.gcp-cluster1.jdpff.gcp.redhatworkshops.io` → `<api-lb-ip>`

Get API load balancer IP:

```bash
gcloud compute addresses list --filter="name~'gcp-cluster1.*api'" --format="value(address)"
```
...
- **Console**: https://console-openshift-console.apps.gcp-cluster1.jdpff.gcp.redhatworkshops.io
- **API**: https://api.gcp-cluster1.jdpff.gcp.redhatworkshops.io:6443
- **OAuth**: https://oauth-openshift.apps.gcp-cluster1.jdpff.gcp.redhatworkshops.io
...
```bash
gcloud compute instances list --filter="labels.kubernetes-io-cluster-gcp-cluster1"
gcloud compute disks list --filter="labels.kubernetes-io-cluster-gcp-cluster1"
...
- [OpenShift GCP Documentation](https://docs.openshift.com/container-platform/4.20/installing/installing_gcp/installing-gcp-customizations.html)
